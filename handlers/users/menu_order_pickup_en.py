...
from keyboards.default_keyboards.confirm_or_delete import con_or_del_en
from keyboards.default_keyboards.category_en import category_button_en, product_button_en


@dp.message_handler(text="🏃🏼 Pickup", content_types=types.ContentTypes.TEXT)
async def delivery(message: types.Message):
    c_b_u = await category_button_en()
    await message.answer("Choose the category\n👇👇👇", reply_markup=c_b_u)
    await Menu_pickup_en.categories.set()


@dp.message_handler(state=Menu_pickup_en.categories, content_types=types.ContentTypes.TEXT)
async def categories(message: types.Message, state: FSMContext):
    if message.text == "⬅ Back":
        ...
        await Menu_pickup_en.products.set()


@dp.message_handler(state=Menu_pickup_en.products, content_types=types.ContentTypes.TEXT)
async def products(message: types.Message, state: FSMContext):
    if message.text == "⬅ Back":
        c_b_u = await category_button_en()
        await message.answer("Choose category\n👇👇👇", reply_markup=c_b_u)
        await Menu_pickup_en.previous()
    else:
        ...
        await Menu_pickup_en.product.set()
...
@dp.callback_query_handler(state=Menu_pickup_en.product)
async def product_total(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "pplus":
        ...
        await Menu_pickup_en.yes_or_no.set()


@dp.message_handler(state=Menu_pickup_en.yes_or_no, content_types=types.ContentTypes.TEXT)
async def yes_or_no(message: types.Message, state: FSMContext):
    ...
    if message.text == "✅ Yes":
        ...
        await Menu_pickup_en.con_or_del.set()


@dp.message_handler(state=Menu_pickup_en.con_or_del, content_types=types.ContentTypes.TEXT)
...
@dp.message_handler(state=Menu_pickup_en.phone_number_order_confirm, content_types=types.ContentTypes.TEXT)
async def new_phone_number_confirm(message: types.Message, state: FSMContext):
    ...
    if yes_or_no_phone_number == "❌ No":
        ...
        await Menu_pickup_en.new_phone_number.set()
        ...
        await message.answer("Your order has been accepted", reply_markup=menu_en)
        await db.delete_cart(telegram_id=message.from_user.id)
        await db.delete_order_ontime(telegram_id=message.from_user.id)
        await state.finish()


@dp.message_handler(state=Menu_pickup_en.new_phone_number, content_types=types.ContentTypes.TEXT)
@dp.message_handler(state=Menu_pickup_en.new_phone_number, content_types=types.ContentTypes.CONTACT)
