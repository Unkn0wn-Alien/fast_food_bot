...
@dp.message_handler(text="🏃🏼 Самовыноз", content_types=types.ContentTypes.TEXT)
async def delivery(message: types.Message):
    c_b_u = await category_button_ru()
    await message.answer("Выберите категория\n👇👇👇", reply_markup=c_b_u)
    await Menu_pickup_ru.categories.set()


@dp.message_handler(state=Menu_pickup_ru.categories, content_types=types.ContentTypes.TEXT)
async def categories(message: types.Message, state: FSMContext):
    if message.text == "⬅ Назад":
        ...
        await Menu_pickup_ru.products.set()


@dp.message_handler(state=Menu_pickup_ru.products, content_types=types.ContentTypes.TEXT)
async def products(message: types.Message, state: FSMContext):
    if message.text == "⬅ Назад":
        c_b_u = await category_button_ru()
        await message.answer("Выберите категория\n👇👇👇", reply_markup=c_b_u)
        await Menu_pickup_ru.previous()
    else:
        ...
        await Menu_pickup_ru.product.set()
...
@dp.callback_query_handler(state=Menu_pickup_ru.product)
async def product_total(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "plyus":
        ...
        await Menu_pickup_ru.yes_or_no.set()


@dp.message_handler(state=Menu_pickup_ru.yes_or_no, content_types=types.ContentTypes.TEXT)
async def yes_or_no(message: types.Message, state: FSMContext):
    ...
    if message.text == "✅ Да":
        ...
        await Menu_pickup_ru.con_or_del.set()


@dp.message_handler(state=Menu_pickup_ru.con_or_del, content_types=types.ContentTypes.TEXT)
...
@dp.message_handler(state=Menu_pickup_ru.phone_number_order_confirm, content_types=types.ContentTypes.TEXT)
async def new_phone_number_confirm(message: types.Message, state: FSMContext):
    ...
    if yes_or_no_phone_number == "❌ Нет":
        ...
        await Menu_pickup_ru.new_phone_number.set()
        ...
        await message.answer("Ваш заказ принято", reply_markup=menu_ru)
        await db.delete_cart(telegram_id=message.from_user.id)
        await db.delete_order_ontime(telegram_id=message.from_user.id)
        await state.finish()


@dp.message_handler(state=Menu_pickup_ru.new_phone_number, content_types=types.ContentTypes.TEXT)
@dp.message_handler(state=Menu_pickup_ru.new_phone_number, content_types=types.ContentTypes.CONTACT)
